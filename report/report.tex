...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    \Procedure{release\_processor}{}
	  \State {call the scheduler to determine the new process}
...
\begin{algorithm}[H]
  ...
  \For {every process in PCB array}
    \If {id of this process matches the process\_id parameter}
  ...
  \Function {k_set_process_priority}{int process_id, int priority}
    \If {priority is within valid range}
      \For {every user process}
        \If {id of this process matches the process\_id parameter}
          \If {new priority is different}
            \If {process is in wrong priority of ReadyPQ}
	...
	\begin{algorithmic}[1]
	  \Function{c_UART0_IRQHandler}{}
	    \If{receive data available}
	      ...
	      \If{g_char_in is the null character}
	        \State\Return
	      \EndIf
	      \State echo_msg = get memory block (non blocking)
	      \If{memory was received for echo_msg}
	      ...
	      \EndIf
	      \If{cur_msg is NULL (has not been created)}
	        \State request a memory block for cur_msg
	        \If{there is no more memory to create cur_msg}
	          \State\Return
	        \EndIf
	      \EndIf

	      \If{user input was a newline or message about to overflow memory block}
